...
class MoveToPregraspNode(Node):
    ...
    def __get_goal_yaw(
        ...
        let (x_o, y_o) be the lift offset in base frame, and Then, we have the following transforms
        ...
        L = - (y_g)cos(theta) + (x_g)sin(theta) + y_o - x_w 
        ...
        if not account_for_offsets:
            return True, np.arctan2(y_g, x_g), None  # arctan2 returns -pi to pi
        ...
        if self.wrist_offset is None:
            # First, get the transform from the base (Arm?) link to the end of the arm
            T = self.controller.get_transform(
