...
# GR-ConvNet 모델 로드를 위해 grconv 경로를 sys.path에 추가
GRCONV_DIR = os.path.join(os.path.dirname(__file__), "..", "nrc_web_teleop_helpers", "grconv")
GRCONV_DIR = os.path.abspath(GRCONV_DIR)
if GRCONV_DIR not in sys.path:
    sys.path.insert(0, GRCONV_DIR)
...
TRAINED_MODELS_DIR = os.path.join(GRCONV_DIR, "trained-models")
DEFAULT_MODEL_PATH = os.path.join(TRAINED_MODELS_DIR, "cornell-randsplit-rgbd-grconvnet3-drop1-ch32/epoch_19_iou_0.98")


def find_available_models():
    """trained-models 디렉토리에서 사용 가능한 모델 파일들을 찾아 (표시이름, 경로) 리스트로 반환한다."""
    models = []
    if not os.path.isdir(TRAINED_MODELS_DIR):
        return models
    for dirn in sorted(os.listdir(TRAINED_MODELS_DIR)):
        dirp = os.path.join(TRAINED_MODELS_DIR, dirn)
...
def load_grconvnet(model_path=DEFAULT_MODEL_PATH, force_cpu=False):
    global model, device
    if torch.cuda.is_available() and not force_cpu:
        device = torch.device("cuda")
    else:
        device = torch.device("cpu")
    print(f"Loading GR-ConvNet model from {model_path} on {device}...")
    model = torch.load(model_path, map_location=device, weights_only=False)
    model.eval()
    in_ch = model.conv1.in_channels
    print(f"Model loaded. (input_channels={in_ch}, {'RGBD' if in_ch >= 4 else 'Depth-only'})")


def get_grconv_results(depth_image, rgb_image=None):
    """Depth(+RGB) 이미지를 GR-ConvNet에 입력하여 q_out, ang(rad), width(px)를 image 크기로 반환한다."""
    if model is None:
        raise RuntimeError("Model not loaded. Call load_grconvnet() first.")

    input_channels = model.conv1.in_channels

    # depth 전처리: mean 차감, clip [-1, 1]
    depth_proc = np.clip((depth_image.astype(np.float32) - depth_image.mean()), -1, 1)

    if input_channels >= 4 and rgb_image is not None:
        # RGB 전처리: float32/255, mean 차감
        rgb_proc = rgb_image.astype(np.float32) / 255.0
        rgb_proc -= rgb_proc.mean()
        # (H, W, 3) -> (3, H, W)
        rgb_channels = rgb_proc.transpose((2, 0, 1))
        # RGBD: (1, 4, H, W) = depth(1) + rgb(3)
        x = np.concatenate([depth_proc[np.newaxis, :, :], rgb_channels], axis=0)
    else:
        # Depth only: (1, 1, H, W)
        x = depth_proc[np.newaxis, :, :]

    x = torch.from_numpy(x[np.newaxis, :, :, :].astype(np.float32)).to(device)
    ...
    q_out = cv2.resize((255.0 * np.clip(q_img, 0.0, 1.0)).astype(np.uint8), image_size, cv2.INTER_AREA)
    ang_out = cv2.resize(ang_img.astype(np.float32), image_size, cv2.INTER_LINEAR)
    width_out = cv2.resize(width_img.astype(np.float32), image_size, cv2.INTER_LINEAR)
...
class GrConvGui:
    def __init__(self, root):
        ...
        self.root = root
        root.title("GR-ConvNet Test GUI")
        ...
        tk.Label(top2, text="Model:").pack(side=tk.LEFT)
        self.available_models = find_available_models()
        model_names = [name for name, _ in self.available_models]
        self.model_var = tk.StringVar()
        self.model_combo = ttk.Combobox(top2, textvariable=self.model_var, values=model_names,
    ...
    def on_model_change(self, _evt):
        """드롭다운에서 모델을 변경하면 새 모델을 로드하고, 이미지가 있으면 재추론한다."""
        idx = self.model_combo.current()
        if idx < 0:
            return
        _, path = self.available_models[idx]
        load_grconvnet(path)
    ...
    def on_pixel_click(self, event):
        ...
        # crop 영역 밖 클릭 시 peak 검색 안 함
        in_crop = True
        if self.crop_tl is not None and self.crop_br is not None:
            r0, c0 = self.crop_tl
            r1, c1 = self.crop_br
            if not (r0 <= row < r1 and c0 <= col < c1):
                in_crop = False
        if in_crop and self.q_out is not None:
            try:
    ...
    def _crop_region(self):
        """crop이 적용되어 있으면 (r0, c0, crop_h, crop_w)를 반환, 아니면 None."""
        if self.crop_tl is not None and self.crop_br is not None:
            r0, c0 = self.crop_tl
            r1, c1 = self.crop_br
    ...
    def _update_peak_label(self, ref_h, ref_w):
        """peak_rc(전체 이미지 좌표) 위치의 q_out/ang/width 값을 peak_label에 표기한다."""
        ...
        if crop is not None:
            cr0, cc0, ch, cw = crop
            qr = min(int((pr - cr0) * qh / ch), qh - 1)
            qc = min(int((pc - cc0) * qw / cw), qw - 1)
        else:
    ...
    def _find_q_peak_near(self, row, col, ref_h, ref_w, radius=40):
        """q_out 좌표계에서 (row, col) 주변 radius 내 최댓값 위치를 전체 이미지 좌표계로 환산해 반환한다."""
        q = self.q_out
        if q is None:
            return None
        qh, qw = q.shape[:2]
        ...
        if crop is not None:
            cr0, cc0, ch, cw = crop
            qr = int((row - cr0) * qh / ch)
            qc = int((col - cc0) * qw / cw)
        else:
            qr = int(row * qh / ref_h)
            qc = int(col * qw / ref_w)
        r0 = max(qr - radius, 0)
        ...
        if crop is not None:
            cr0, cc0, ch, cw = crop
            peak_row = int(peak_qr * ch / qh) + cr0
            peak_col = int(peak_qc * cw / qw) + cc0
        else:
    ...
    def _refresh_input_display(self):
        ...
        if self.peak_rc is not None:
            ...
            if self.ang_out is not None and self.width_out is not None:
                ...
                if crop is not None:
                    cr0, cc0, ch, cw = crop
                    ar = min(int((r - cr0) * ah / ch), ah - 1)
                    ac = min(int((c - cc0) * aw / cw), aw - 1)
                else:
    ...
    def run(self):
        """전체 RGB로 depth를 예측하고 전체 pred depth로 GR-ConvNet을 실행해 결과를 표시한다."""
        if self.color_image is None:
            return
        t0 = time.time()
        depth_image = get_pred_depth(self.color_image, ref_rcz=self.ref_rcz)
        print(f"Depth Anything prediction: {time.time() - t0:.3f}s")
        self.full_pred_depth = depth_image
        self._run_grconv(depth_image, self.color_image)

    def run_crop(self):
        """crop 영역의 pred depth로 GR-ConvNet을 재추론하여 결과를 표시한다."""
        ...
        self._run_grconv(cropped_depth, cropped_color)

    def _run_grconv(self, depth_image, color_image):
        """depth_image(+color_image)를 GR-ConvNet에 입력하고 depth/overlay 패널을 갱신한다."""
        ...
        t0 = time.time()
        q_out, depth_out, ang_out, width_out = get_grconv_results(
            normalized_depth_image, rgb_image=color_image
        )
        print(f"GR-ConvNet inference: {time.time() - t0:.3f}s")
